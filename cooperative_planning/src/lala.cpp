...
double target_position[3][1];
double target_velocity[3][1];
double target_yaw;
...
void targetDesiredPosReached(ros::Publisher pub, double x, double y, double z){
    ...
    if ( (abs(target_position[0][0] + x- target_inform_point[0][0]) < proximity_radius_target) && (abs(target_position[1][0] + y - target_inform_point[1][0]) < proximity_radius_target) )
...
void targetIsCloseWarning(ros::Publisher pub, double x, double y, double z){
    ...
    if ( (abs(target_position[0][0] + x- shuttle->ekf.pos[0][0]) < target_shuttle_horizontal_closeness) && (abs(target_position[1][0] + y- shuttle->ekf.pos[1][0]) < target_shuttle_horizontal_closeness) )//&& (abs(target->ekf.pos[2][0] + z - shuttle->ekf.pos[2][0]) < target_shuttle_vertical_closeness) )
...
void mpcController(ros::Publisher pub, Function mpc,double relative_x, double relative_y, double relative_z){
    ...
    std::vector<double> xx0 = {  shuttle->ekf.pos[0][0] ,shuttle->ekf.pos[1][0],shuttle->ekf.pos[2][0],  shuttle->ekf.vel[0][0],shuttle->ekf.vel[1][0],shuttle->ekf.vel[2][0],  shuttle->ekf.att_euler[3][0],     target_position[0][0] + relative_x ,target_position[1][0] + relative_y,target_position[2][0] + relative_z ,  target_velocity[0][0],target_velocity[1][0],target_velocity[2][0],  target_yaw + 1.3962634 }; //shuttle and target states for mpc
    ...
    casadi::Matrix<double> result_matrix2 = res.at(0);
    ...
    double psi = (double)result_matrix2(3,1);
...
*/
  ...
  std::string line;
        ...
        if(std::getline(myfile, line)){
            ...
            target_position[0][0] = result[0];
            target_position[1][0]= result[1];
            target_position[2][0]= result[2];
            target_velocity[0][0] = result[3];
            target_velocity[1][0]= result[4];
            target_velocity[2][0] = result[5];
            target_yaw = result[6];
       ...
       // ROS_WARN_STREAM("FILE READ"  );


        }
        else{
           // ROS_WARN_STREAM("ENDED READING FILE"  );
            myfile.close();
