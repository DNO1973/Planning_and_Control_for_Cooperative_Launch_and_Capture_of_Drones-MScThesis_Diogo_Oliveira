...
int main (int argc, char ** argv){
  ...
  // file name
    ...
    while(ros::ok() ){
        ...
        myfile.open (lib_name, std::ios::app);
      myfile  << shuttle->ekf.pos[0][0]  << "," <<  shuttle->ekf.pos[1][0]  << "," <<  shuttle->ekf.pos[2][0] << "," <<   shuttle->ekf.vel[0][0] << "," << shuttle->ekf.vel[1][0] << "," << shuttle->ekf.vel[2][0] << "," <<   shuttle->ekf.att_euler[3][0]   << "," <<  target->ekf.pos[0][0] + referential_relative_to_shuttle_x << "," << target->ekf.pos[1][0] + referential_relative_to_shuttle_y << "," << target->ekf.pos[2][0] + referential_relative_to_shuttle_z << "," <<  target->ekf.vel[0][0] << "," << target->ekf.vel[1][0] << "," << target->ekf.vel[2][0] << "," <<  target->ekf.att_euler[3][0] + 1.3962634 <<  "\n";
    ROS_WARN("logging...");
